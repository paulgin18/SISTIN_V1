...
class Dispositivo extends TableGateway {
	...
	public function buscarDispositivo($descripcion, $tipo) {
		...
		$sql="";
		if(isset($v[1])){ 
			$sql="SELECT id_disp_soft as value, descripcion as label, * FROM disp_soft where descripcion like '%$descripcion%' and tipo='$v[0]'";
		}else{
			$sql="SELECT id_disp_soft as value, descripcion as label, * FROM disp_soft where descripcion like '%$descripcion%' and tipo='$tipo'";
		}
	...
	public function bMaMoxDisp($descripcion, $id_disp) {
		...
		$consulta = $this->dbAdapter->query(
				...
				. " where (m.descripcion || ' / ' || mo.descripcion) like '%$descripcion%' and dpmm.vigencia='true' and id_disp_soft=$id_disp ", Adapter::QUERY_MODE_EXECUTE);
		$datos = $consulta->toArray();
		return $datos;
	}
	...
	public function bMaxDisp($descripcion, $id_disp) {
		$consulta = $this->dbAdapter->query(
				...
				. " inner join marca m on dpmm.id_marca=m.id_marca"
				. " inner join modelo mo on dpmm.id_modelo=mo.id_modelo"
				. " where m.descripcion like '%$descripcion%' and dpmm.vigencia='true' and id_disp_soft=$id_disp", Adapter::QUERY_MODE_EXECUTE);
		$datos = $consulta->toArray();
		return $datos;
	}
	...
	public function bSofDisp($descripcion, $tipo) {
		...
		$sql = "Select id_disp_mar_mod as value, s.descripcion as label, s.tipo as tipo from disp_mar_mod dpmm "
				...
				. " where s.descripcion like '%$descripcion%' and  dpmm.vigencia='true' and ". $sql2;
