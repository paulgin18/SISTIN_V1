...
!function (a, b) {
  ...
  p.prompt = function () {
    ...
    a.show = !1,
    !a.title) throw new Error('prompt requires a title'); if (!b.isFunction(a.callback)) throw new Error('prompt requires a callback'); if (!n.inputs[a.inputType]) throw new Error('invalid prompt type'); switch (h = b(n.inputs[a.inputType]), a.inputType) {
      case 'text':
      case 'textarea':
      case 'email':
      case 'date':
      ...
      case 'select':
        ...
        if (k = a.inputOptions || [
        ], !b.isArray(k)) throw new Error('Please pass an array of input options');
      ...
      case 'checkbox':
        ...
        if (k = a.inputOptions || [
        ], !k.length) throw new Error('prompt with checkbox requires options');
